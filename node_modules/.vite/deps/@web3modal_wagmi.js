...
import {
  ...
  trim,
  weiUnits
} from "./chunk-PHBTKJCJ.js";
import {
  equalBytes
} from "./chunk-MEUPPBR3.js";
import "./chunk-2B3V2GUC.js";
import {
  UiHelperUtil,
  setColorTheme,
  setThemeVariables
} from "./chunk-6EKAYA7L.js";
import {
  LitElement,
  css,
  customElement,
...
function createFilterRequestScope(client, { method }) {
  ...
  if (client.transport.type === "fallback")
    (_b = (_a2 = client.transport).onResponse) == null ? void 0 : _b.call(_a2, ({ method: method_, response: id2, status, transport }) => {
...
function getContractError(err, { abi, address, args, docsPath: docsPath3, functionName, sender }) {
  ...
  } else if ([EXECUTION_REVERTED_ERROR_CODE, InternalRpcError.code].includes(code) && (data3 || message || shortMessage)) {
...
async function getBlock(client, { blockHash, blockNumber, blockTag: blockTag_, includeTransactions: includeTransactions_ } = {}) {
  ...
  const format = ((_c = (_b = (_a2 = client.chain) == null ? void 0 : _a2.formatters) == null ? void 0 : _b.block) == null ? void 0 : _c.format) || formatBlock;
  return format(block);
}
...
async function estimateGas(client, args) {
  ...
  try {
    ...
    assertRequest(args);
    const format = ((_c = (_b = (_a2 = client.chain) == null ? void 0 : _a2.formatters) == null ? void 0 : _b.transactionRequest) == null ? void 0 : _c.format) || formatTransactionRequest;
...
function decodeEventLog({ abi, data: data3, strict: strict_, topics }) {
...
async function readContract(client, { abi, address, args, functionName, ...callRequest }) {
  ...
  try {
    const { data: data3 } = await call(client, {
      data: calldata,
      to: address,
      ...callRequest
    });
  ...
  for (const key in callbacks) {
    emit[key] = (...args) => {
      ...
      listeners2.forEach((listener) => {
        var _a2, _b;
        return (_b = (_a2 = listener.fns)[key]) == null ? void 0 : _b.call(_a2, ...args);
      });
    };
  }
...
function poll(fn, { emitOnBegin, initialWaitTime, interval }) {
  ...
  const watch = async () => {
    ...
    const initialWait = await (initialWaitTime == null ? void 0 : initialWaitTime(data3)) ?? interval;
...
function getCache(cacheKey2) {
  const buildCache = (cacheKey3, cache) => ({
    ...
    set: (data3) => cache.set(cacheKey3, data3)
  });
...
async function withCache(fn, { cacheKey: cacheKey2, cacheTime = Infinity }) {
  ...
  try {
    ...
    cache.response.set({ created: /* @__PURE__ */ new Date(), data: data3 });
    return data3;
  } finally {
    cache.promise.clear();
  }
...
async function sendTransaction(client, args) {
  var _a2, _b, _c;
  const { account: account_ = client.account, chain = client.chain, accessList, data: data3, gas, gasPrice, maxFeePerGas, maxPriorityFeePerGas, nonce, to, value, ...rest } = args;
  if (!account_)
    throw new AccountNotFoundError({
...
function webSocket(socket, { body, onResponse }) {
  ...
  const callback = ({ data: data3 }) => {
    ...
    if (body.method === "eth_unsubscribe") {
      socket.subscriptions.delete((_a2 = body.params) == null ? void 0 : _a2[0]);
    }
  };
...
async function getTransaction(client, { blockHash, blockNumber, blockTag: blockTag_, hash, index: index2 }) {
  ...
  const format = ((_c = (_b = (_a2 = client.chain) == null ? void 0 : _a2.formatters) == null ? void 0 : _b.transaction) == null ? void 0 : _c.format) || formatTransaction;
...
async function getTransactionReceipt(client, { hash }) {
  ...
  const format = ((_c = (_b = (_a2 = client.chain) == null ? void 0 : _a2.formatters) == null ? void 0 : _b.transactionReceipt) == null ? void 0 : _c.format) || formatTransactionReceipt;
...
function encodeData({ data: data3, primaryType, types }) {
      ...
      try {
        const { unsubscribe: unsubscribe_ } = await client.transport.subscribe({
          ...
          onData(data3) {
            ...
            if (!active)
              return;
            const blockNumber = hexToBigInt((_a2 = data3.result) == null ? void 0 : _a2.number);
        ...
        const { unsubscribe: unsubscribe_ } = await client.transport.subscribe({
          ...
          onData(data3) {
            ...
            if (!active)
              return;
            const format = ((_c = (_b = (_a2 = client.chain) == null ? void 0 : _a2.formatters) == null ? void 0 : _b.block) == null ? void 0 : _c.format) || formatBlock;
            const block = format(data3.result);
      ...
      try {
        const { unsubscribe: unsubscribe_ } = await client.transport.subscribe({
          ...
          onData(data3) {
            if (!active)
              return;
            const transaction = data3.result;
...
function webSocket2(url, config2 = {}) {
  ...
  return ({ chain, retryCount: retryCount_, timeout: timeout_ }) => {
    ...
    const retryCount = config2.retryCount ?? retryCount_;
    const timeout = timeout_ ?? config2.timeout ?? 1e4;
    const url_ = url || ((_a2 = chain == null ? void 0 : chain.rpcUrls.default.webSocket) == null ? void 0 : _a2[0]);
    if (!url_)
      throw new UrlRequiredError();
    return createTransport({
      key,
...
var InjectedConnector = class extends Connector {
  constructor({
    ...
    this.onDisconnect = async (error) => {
      ...
      if (this.options.shimDisconnect)
        (_a2 = this.storage) == null ? void 0 : _a2.removeItem(this.shimDisconnectKey);
    };
  ...
  async connect({ chainId } = {}) {
    ...
    try {
      ...
      if (this.options.shimDisconnect)
        (_a2 = this.storage) == null ? void 0 : _a2.setItem(this.shimDisconnectKey, true);
  ...
  async isAuthorized() {
    var _a2;
    try {
      if (this.options.shimDisconnect && !((_a2 = this.storage) == null ? void 0 : _a2.getItem(this.shimDisconnectKey)))
        return false;
      const provider = await this.getProvider();
      if (!provider)
        throw new ConnectorNotFoundError();
  ...
  async switchChain(chainId) {
    ...
    try {
      ...
      if (error.code === 4902 || ((_b = (_a2 = error == null ? void 0 : error.data) == null ? void 0 : _a2.originalError) == null ? void 0 : _b.code) === 4902) {
        try {
...
var createStoreImpl = (createState) => {
  ...
  const setState = (partial, replace) => {
    ...
    if (!Object.is(nextState, state2)) {
      ...
      state2 = (replace != null ? replace : typeof nextState !== "object") ? nextState : Object.assign({}, state2, nextState);
...
function deserialize(cachedString) {
  ...
  const deserializedCacheWithBigInts = findAndReplace(cache, {
    find: (data3) => typeof data3 === "string" && data3.startsWith("#bigint."),
    replace: (data3) => BigInt(data3.replace("#bigint.", ""))
  });
...
var Config = class {
  constructor({
    ...
    if (autoConnect) {
      try {
        ...
        const data3 = (_a2 = rawState == null ? void 0 : rawState.state) == null ? void 0 : _a2.data;
        status = (data3 == null ? void 0 : data3.account) ? "reconnecting" : "connecting";
        chainId = (_b = data3 == null ? void 0 : data3.chain) == null ? void 0 : _b.id;
      } catch (_error) {
      }
    }
          ...
          }),
          {
            ...
            partialize: (state2) => {
              ...
              return {
                ...autoConnect && {
                  data: {
                    account: (_a3 = state2 == null ? void 0 : state2.data) == null ? void 0 : _a3.account,
                    chain: (_b2 = state2 == null ? void 0 : state2.data) == null ? void 0 : _b2.chain
                  }
                },
                chains: state2 == null ? void 0 : state2.chains
              };
            },
            version: 2
          }
  ...
  get lastUsedChainId() {
    var _a2, _b;
    return (_b = (_a2 = this.data) == null ? void 0 : _a2.chain) == null ? void 0 : _b.id;
  }
  get publicClient() {
  ...
  async destroy() {
    var _a2, _b;
    if (this.connector)
      await ((_b = (_a2 = this.connector).disconnect) == null ? void 0 : _b.call(_a2));
  ...
  async autoConnect() {
    ...
    this.setState((x) => {
      ...
      return {
        ...x,
        status: ((_a2 = x.data) == null ? void 0 : _a2.account) ? "reconnecting" : "connecting"
      };
    });
    ...
    for (const connector of sorted) {
      ...
      if (!isAuthorized)
        continue;
      const data3 = await connector.connect();
      this.setState((x) => ({
        ...x,
        connector,
  ...
  setPublicClient(publicClient) {
    var _a2, _b;
    const chainId = (_b = (_a2 = this.data) == null ? void 0 : _a2.chain) == null ? void 0 : _b.id;
    this.args = {
      ...this.args,
      publicClient
    };
  ...
  setLastUsedConnector(lastUsedConnector = null) {
    var _a2;
    (_a2 = this.storage) == null ? void 0 : _a2.setItem("wallet", lastUsedConnector);
  }
};
...
addEffects_fn = function() {
  ...
  this.store.subscribe(
    ...
    (connector, prevConnector) => {
      ...
      (_a2 = prevConnector == null ? void 0 : prevConnector.off) == null ? void 0 : _a2.call(prevConnector, "change", onChange);
  ...
  if (subscribePublicClient || subscribeWebSocketPublicClient)
    this.store.subscribe(
      ({ data: data3 }) => {
        var _a2;
        return (_a2 = data3 == null ? void 0 : data3.chain) == null ? void 0 : _a2.id;
      },
      (chainId) => {
        this.setState((x) => ({
          ...x,
...
async function connect({ chainId, connector }) {
  ...
  try {
    ...
    const data3 = await connector.connect({ chainId });
...
function getAccount() {
  const { data: data3, connector, status } = getConfig();
  switch (status) {
    case "connected":
      return {
        address: data3 == null ? void 0 : data3.account,
        connector,
        isConnected: true,
        isConnecting: false,
    ...
    case "reconnecting":
      return {
        address: data3 == null ? void 0 : data3.account,
        connector,
        isConnected: !!(data3 == null ? void 0 : data3.account),
        isConnecting: false,
        isDisconnected: false,
        isReconnecting: true,
        status
...
function getNetwork() {
  var _a2, _b, _c, _d;
  const config2 = getConfig();
  const chainId = (_b = (_a2 = config2.data) == null ? void 0 : _a2.chain) == null ? void 0 : _b.id;
...
function watchAccount(callback, { selector = (x) => x } = {}) {
  ...
  const unsubscribe = config2.subscribe(
    ({ data: data3, connector, status }) => selector({
      address: data3 == null ? void 0 : data3.account,
      connector,
      status
    }),
...
function watchNetwork(callback, { selector = (x) => x } = {}) {
  ...
  const unsubscribe = config2.subscribe(
    ({ data: data3, chains }) => {
      var _a2;
      return selector({ chainId: (_a2 = data3 == null ? void 0 : data3.chain) == null ? void 0 : _a2.id, chains });
    },
    handleChange,
    {
      equalityFn: shallow
        ...
        <wui-flex flexDirection="column" alignItems="center">
          <wui-flex gap="3xs" alignItems="center" justifyContent="center">
            <wui-text variant="large-600" color="fg-100">
              ${this.profileName ? UiHelperUtil.getTruncateString(this.profileName, 20, "end") : UiHelperUtil.getTruncateString(this.address, 8, "middle")}
            </wui-text>
            <wui-icon-link
              size="md"
              icon="copy"
...
var W3mConnectView = class W3mConnectView2 extends LitElement {
  ...
  recommendedTemplate() {
    ...
    const { customWallets } = OptionsController.state;
    if (!recommended.length || featured.length || (customWallets == null ? void 0 : customWallets.length)) {
      return null;
    }
...
W3mNetworkSwitchView = __decorate13([
...
W3mWhatIsANetworkView = __decorate15([
...
W3mWhatIsAWalletView = __decorate16([
...
W3mAllWalletsList = __decorate17([
...
W3mAllWalletsSearch = __decorate18([
...
W3mConnectingHeader = __decorate19([
...
W3mConnectingWcDesktop = __decorate20([
  customElement("w3m-connecting-wc-desktop")
], W3mConnectingWcDesktop);

// node_modules/@web3modal/scaffold/dist/esm/src/partials/w3m-connecting-wc-injected/index.js
...
var W3mConnectingWcInjected = class W3mConnectingWcInjected2 extends W3mConnectingWidget {
  constructor() {
    super();
    if (!this.wallet) {
...
W3mConnectingWcInjected = __decorate21([
...
W3mConnectingWcMobile = __decorate22([
...
W3mConnectingWcQrcode = __decorate23([
...
W3mConnectingWcUnsupported = __decorate24([
...
W3mConnectingWcWeb = __decorate25([
...
W3mSnackBar.styles = styles_default11;
__decorate29([
  state()
], W3mSnackBar.prototype, "open", void 0);
W3mSnackBar = __decorate29([
  ...
  async initOrContinue() {
    if (!this.initPromise && !isInitialized && CoreHelperUtil.isClient()) {
      ...
      this.initPromise = new Promise(async (resolve) => {
        await Promise.all([import("./esm-C4QVNEZ4.js"), import("./w3m-modal-VWD5PXNH.js")]);
...
function getCaipTokens(tokens) {
  ...
  Object.entries(tokens).forEach(([id2, token]) => {
    caipTokens[`${NAMESPACE}:${id2}`] = token;
  });
  return caipTokens;
}
...
var Web3Modal = class extends Web3ModalScaffold {
  constructor(options) {
    ...
    const connectionControllerClient = {
      ...
      connectExternal: async (id2) => {
        var _a2;
        const connector = wagmiConfig.connectors.find((c) => c.id === id2);
        if (!connector) {
  ...
  syncRequestedNetworks(chains, chainImages) {
    const requestedCaipNetworks = chains == null ? void 0 : chains.map((chain) => ({
      ...
      name: chain.name,
      imageId: NetworkImageIds[chain.id],
      imageUrl: chainImages == null ? void 0 : chainImages[chain.id]
    }));
    this.setRequestedCaipNetworks(requestedCaipNetworks ?? []);
  }
  async syncAccount() {
  ...
  async syncNetwork(chainImages) {
    ...
    if (chain) {
      ...
      if (isConnected && address) {
        const caipAddress = `${NAMESPACE}:${chain.id}:${address}`;
        this.setCaipAddress(caipAddress);
        if ((_b = (_a2 = chain.blockExplorers) == null ? void 0 : _a2.default) == null ? void 0 : _b.url) {
  ...
  syncConnectors(connectors) {
    const w3mConnectors = connectors.map(({ id: id2, name }) => ({
      id: id2,
      explorerId: ConnectorExplorerIds[id2],
      imageId: ConnectorImageIds[id2],
      name: ConnectorNamesMap[id2] ?? name,
      type: ConnectorTypesMap[id2] ?? "EXTERNAL"
    }));
...
var CoinbaseWalletConnector = class extends Connector {
  ...
  async getProvider() {
    var _a2;
    if (!__privateGet(this, _provider2)) {
      let CoinbaseWalletSDK = (await import("./dist-VU4PUAVV.js")).default;
...
initProvider_fn = async function() {
  const { EthereumProvider, OPTIONAL_EVENTS, OPTIONAL_METHODS } = await import("./index.es-PHXPTDYB.js");
  ...
  if (defaultChain) {
    ...
    __privateSet(this, _provider3, await EthereumProvider.init({
      showQrModal,
      qrModalOptions,
      projectId,
...
setupListeners_fn = function() {
  if (!__privateGet(this, _provider3))
    return;
  __privateMethod(this, _removeListeners, removeListeners_fn).call(this);
  __privateGet(this, _provider3).on("accountsChanged", this.onAccountsChanged);
  __privateGet(this, _provider3).on("chainChanged", this.onChainChanged);
  __privateGet(this, _provider3).on("disconnect", this.onDisconnect);
  __privateGet(this, _provider3).on("session_delete", this.onDisconnect);
  __privateGet(this, _provider3).on("display_uri", this.onDisplayUri);
  __privateGet(this, _provider3).on("connect", this.onConnect);
};
_removeListeners = /* @__PURE__ */ new WeakSet();
removeListeners_fn = function() {
  if (!__privateGet(this, _provider3))
    return;
  __privateGet(this, _provider3).removeListener("accountsChanged", this.onAccountsChanged);
  __privateGet(this, _provider3).removeListener("chainChanged", this.onChainChanged);
  __privateGet(this, _provider3).removeListener("disconnect", this.onDisconnect);
  __privateGet(this, _provider3).removeListener("session_delete", this.onDisconnect);
  __privateGet(this, _provider3).removeListener("display_uri", this.onDisplayUri);
  __privateGet(this, _provider3).removeListener("connect", this.onConnect);
};
...
getNamespaceChainsIds_fn = function() {
  ...
  if (!__privateGet(this, _provider3))
    return [];
  const namespaces = (_a2 = __privateGet(this, _provider3).session) == null ? void 0 : _a2.namespaces;
  if (!namespaces)
    return [];
...
function defaultWagmiConfig({ projectId, chains, appName }) {
  const { publicClient } = configureChains(chains, [walletConnectProvider({ projectId }), publicProvider()], { retryCount: 1, stallTimeout: 3e3 });
  return createConfig({
    autoConnect: true,
    connectors: [
