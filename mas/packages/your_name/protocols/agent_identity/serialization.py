...
from packages.your_name.protocols.agent_identity import (  # type: ignore
    agent_identity_pb2,
)
from packages.your_name.protocols.agent_identity.custom_types import (  # type: ignore
    ErrorCode,
    EthAddress,
)
from packages.your_name.protocols.agent_identity.message import (  # type: ignore
...
class AgentIdentitySerializer(Serializer):
    ...
    def encode(msg: Message) -> bytes:
        ...
        if performative_id == AgentIdentityMessage.Performative.REQUEST_IDENTITY:
            performative = agent_identity_pb2.AgentIdentityMessage.Request_Identity_Performative()  # type: ignore
            identity_type = msg.identity_type
            performative.identity_type = identity_type
            agent_identity_msg.request_identity.CopyFrom(performative)
        elif performative_id == AgentIdentityMessage.Performative.PROVIDE_IDENTITY:
            performative = agent_identity_pb2.AgentIdentityMessage.Provide_Identity_Performative()  # type: ignore
            ethereum_public_address = msg.ethereum_public_address
            EthAddress.encode(performative.ethereum_public_address, ethereum_public_address)
            ...
            agent_identity_msg.provide_identity.CopyFrom(performative)
    ...
    def decode(obj: bytes) -> Message:
        ...
        if performative_id == AgentIdentityMessage.Performative.REQUEST_IDENTITY:
            identity_type = agent_identity_pb.request_identity.identity_type
            performative_content["identity_type"] = identity_type
        elif performative_id == AgentIdentityMessage.Performative.PROVIDE_IDENTITY:
            pb2_ethereum_public_address = agent_identity_pb.provide_identity.ethereum_public_address
            ethereum_public_address = EthAddress.decode(pb2_ethereum_public_address)
            performative_content["ethereum_public_address"] = ethereum_public_address
            if agent_identity_pb.provide_identity.solana_public_address_is_set:
                solana_public_address = agent_identity_pb.provide_identity.solana_public_address
                performative_content["solana_public_address"] = solana_public_address
            if agent_identity_pb.provide_identity.cosmos_public_address_is_set:
                cosmos_public_address = agent_identity_pb.provide_identity.cosmos_public_address
                performative_content["cosmos_public_address"] = cosmos_public_address
            if agent_identity_pb.provide_identity.gpg_is_set:
                gpg = agent_identity_pb.provide_identity.gpg
                performative_content["gpg"] = gpg
            if agent_identity_pb.provide_identity.rsa_is_set:
                rsa = agent_identity_pb.provide_identity.rsa
                performative_content["rsa"] = rsa
            if agent_identity_pb.provide_identity.role_is_set:
                role = agent_identity_pb.provide_identity.role
                performative_content["role"] = role
            if agent_identity_pb.provide_identity.public_endpoint_is_set:
                public_endpoint = agent_identity_pb.provide_identity.public_endpoint
                performative_content["public_endpoint"] = public_endpoint
            if agent_identity_pb.provide_identity.private_endpoint_is_set:
                private_endpoint = agent_identity_pb.provide_identity.private_endpoint
                performative_content["private_endpoint"] = private_endpoint
            if agent_identity_pb.provide_identity.public_ws_endpoint_is_set:
                public_ws_endpoint = agent_identity_pb.provide_identity.public_ws_endpoint
                performative_content["public_ws_endpoint"] = public_ws_endpoint
            if agent_identity_pb.provide_identity.private_ws_endpoint_is_set:
                private_ws_endpoint = agent_identity_pb.provide_identity.private_ws_endpoint
