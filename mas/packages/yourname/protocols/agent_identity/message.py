...
from packages.yourname.protocols.agent_identity.custom_types import (
    ErrorCode as CustomErrorCode,
)


_default_logger = logging.getLogger("aea.packages.yourname.protocols.agent_identity.message")

DEFAULT_BODY_SIZE = 4


class AgentIdentityMessage(Message):
    """A protocol for representing an agent with various public and private identities."""

    protocol_id = PublicId.from_str("yourname/agent_identity:0.1.0")
    protocol_specification_id = PublicId.from_str("yourname/agent_identity:0.1.0")
    ...
    _performatives = {"error", "get_identity", "identity_response", "set_identity"}
    ...
    @property
    def ethereum_public_address(self) -> str:
        ...
        return cast(str, self.get("ethereum_public_address"))
    ...
    def _is_consistent(self) -> bool:
        ...
        try:
            ...
            if self.performative == AgentIdentityMessage.Performative.SET_IDENTITY:
                expected_nb_of_contents = 1
                enforce(
                    isinstance(self.ethereum_public_address, str),
                    "Invalid type for content 'ethereum_public_address'. Expected 'str'. Found '{}'.".format(
            ...
            elif self.performative == AgentIdentityMessage.Performative.GET_IDENTITY:
                expected_nb_of_contents = 1
                enforce(
            ...
            elif self.performative == AgentIdentityMessage.Performative.IDENTITY_RESPONSE:
                expected_nb_of_contents = 1
                enforce(
                    isinstance(self.ethereum_public_address, str),
                    "Invalid type for content 'ethereum_public_address'. Expected 'str'. Found '{}'.".format(
