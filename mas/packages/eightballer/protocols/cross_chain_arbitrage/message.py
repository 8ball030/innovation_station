...
from packages.eightballer.protocols.cross_chain_arbitrage.custom_types import (
    RejectionReason as CustomRejectionReason,
)


_default_logger = logging.getLogger("aea.packages.eightballer.protocols.cross_chain_arbitrage.message")

DEFAULT_BODY_SIZE = 4


class CrossChainArbitrageMessage(Message):
    """A protocol for managing cross-chain arbitrage activities between an arbitrageur and multiple exchanges."""

    protocol_id = PublicId.from_str("eightballer/cross_chain_arbitrage:0.1.0")
    protocol_specification_id = PublicId.from_str("eightballer/cross_chain_arbitrage:0.1.0")
    ...
    def buy_price(self) -> float:
        """Get the 'buy_price' content from the message."""
        enforce(self.is_set("buy_price"), "'buy_price' content is not set.")
        return cast(float, self.get("buy_price"))

    @property
    def buy_side_exchange_id(self) -> str:
        """Get the 'buy_side_exchange_id' content from the message."""
        enforce(self.is_set("buy_side_exchange_id"), "'buy_side_exchange_id' content is not set.")
        return cast(str, self.get("buy_side_exchange_id"))
    ...
    def note(self) -> Optional[str]:
        """Get the 'note' content from the message."""
        return cast(Optional[str], self.get("note"))
    ...
    @property
    def reason(self) -> CustomRejectionReason:
        """Get the 'reason' content from the message."""
        enforce(self.is_set("reason"), "'reason' content is not set.")
        return cast(CustomRejectionReason, self.get("reason"))

    @property
    def sell_price(self) -> float:
        """Get the 'sell_price' content from the message."""
        enforce(self.is_set("sell_price"), "'sell_price' content is not set.")
        return cast(float, self.get("sell_price"))

    @property
    def sell_side_exchange_id(self) -> str:
        """Get the 'sell_side_exchange_id' content from the message."""
        enforce(self.is_set("sell_side_exchange_id"), "'sell_side_exchange_id' content is not set.")
        return cast(str, self.get("sell_side_exchange_id"))

    @property
    def token_a_address(self) -> str:
        """Get the 'token_a_address' content from the message."""
        enforce(self.is_set("token_a_address"), "'token_a_address' content is not set.")
        return cast(str, self.get("token_a_address"))

    @property
    def token_b_address(self) -> str:
        """Get the 'token_b_address' content from the message."""
        enforce(self.is_set("token_b_address"), "'token_b_address' content is not set.")
        return cast(str, self.get("token_b_address"))
    ...
    def _is_consistent(self) -> bool:
        ...
        try:
            ...
            if self.performative == CrossChainArbitrageMessage.Performative.ARBITRAGE_OPPORTUNITY:
                ...
                enforce(
                    isinstance(self.buy_side_exchange_id, str),
                    "Invalid type for content 'buy_side_exchange_id'. Expected 'str'. Found '{}'.".format(
                        type(self.buy_side_exchange_id)
                    ),
                )
                enforce(
                    isinstance(self.sell_side_exchange_id, str),
                    "Invalid type for content 'sell_side_exchange_id'. Expected 'str'. Found '{}'.".format(
                        type(self.sell_side_exchange_id)
                    ),
                )
                enforce(
                ...
                enforce(
                    isinstance(self.buy_price, float),
                    "Invalid type for content 'buy_price'. Expected 'float'. Found '{}'.".format(type(self.buy_price)),
                )
                enforce(
                    isinstance(self.sell_price, float),
                    "Invalid type for content 'sell_price'. Expected 'float'. Found '{}'.".format(
                        type(self.sell_price)
                    ),
                )
                if self.is_set("note"):
                    ...
                    enforce(
                        isinstance(note, str),
                        "Invalid type for content 'note'. Expected 'str'. Found '{}'.".format(type(note)),
                    )
            elif self.performative == CrossChainArbitrageMessage.Performative.REJECTED:
                expected_nb_of_contents = 1
                enforce(
                    isinstance(self.reason, CustomRejectionReason),
                    "Invalid type for content 'reason'. Expected 'RejectionReason'. Found '{}'.".format(
                        type(self.reason)
                    ),
                )
                ...
                enforce(
                    isinstance(self.token_a_address, str),
                    "Invalid type for content 'token_a_address'. Expected 'str'. Found '{}'.".format(
                        type(self.token_a_address)
                    ),
                )
                ...
                enforce(
                    isinstance(self.token_b_address, str),
                    "Invalid type for content 'token_b_address'. Expected 'str'. Found '{}'.".format(
                        type(self.token_b_address)
                    ),
                )
